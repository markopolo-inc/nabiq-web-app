  ...
  return (
    <Stack gap={32}>
      {configs.map((item, idx) => (
        <Stack gap={8} key={idx}>
          <p className='text-gray-600 font-normal text-sm'>{formatTimeAgo(item.queuedAt)}</p>
            ...
            <Stack gap={20}>
              ...
              {item?.status === 'processing' ? (
                <div className='flex flex-nowrap items-center justify-between'>
                  ...
                  <span className='text-gray-700 font-sm font-medium'>{item?.timeLeft} left</span>
                </div>
              ) : (
                <Badge color='gray'>
            ...
            {item?.status === 'processing' && (
              <Group>
                <Button
                  ...
                  onClick={() => navigate(`/control-room/cohort/content/${item?.id}`)}
                >
                  View content sample
                </Button>
                <Button
