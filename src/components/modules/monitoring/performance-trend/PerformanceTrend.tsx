...
export const PerformanceTrend: FC<{
  ...
  if (error) return <p className='text-red-600'>Error loading performance data</p>;
  if (graphData?.length === 0) return <p className='text-gray-600'>No data available!</p>;
            ...
            <div key={metric} className='flex items-center'>
              <div
                className={`w-2.5 h-2.5 rounded-full mr-2 ${
                  ['bg-blue-500', 'bg-pink-500', 'bg-purple-500', 'bg-green-500', 'bg-yellow-400'][
                    index
                  ]
                }`}
              />
