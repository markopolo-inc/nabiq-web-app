...
import {
  ...
  PassResetSuccess,
  ResetPass,
} from 'src/components/modules/reset-pass';
      ...
      <div className='min-h-screen pt-14 w-full flex items-center justify-center'>
        <Stack className='max-w-[468px] w-full mx-auto' align='center' gap={24}>
          {step === ResetPasswordStep.EMAIL_INPUT && (
            <ResetPass onSetup={() => setStep(ResetPasswordStep.RESEND_CODE)} />
          )}
          {step === ResetPasswordStep.RESEND_CODE && (
            <CheckEmail onSetup={() => setStep(ResetPasswordStep.NEW_PASSWORD)} />
          )}
          {step === ResetPasswordStep.NEW_PASSWORD && (
            <NewPass onSetup={() => setStep(ResetPasswordStep.SUCCESS)} />
          )}
